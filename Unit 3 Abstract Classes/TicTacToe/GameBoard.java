...
public class GameBoard extends Canvas implements MouseListener
{
	...
	public void markBoard()
	{
		if(mouseX>=WIDTH/3&&mouseX<=WIDTH+50&&mouseY>=HEIGHT/3&&mouseY<=HEIGHT+50)
		{
			...
			if(prevMouseButton!=mouseButton)		//left mouse button pressed
			{
				if(piece==null)
				{
					//board.setSpot(r,c,new Piece(5+c*50+50,5+r*50+50,WIDTH/3-10,HEIGHT/3-10,"X",Color.GREEN));
					if (MouseEvent.BUTTON1 == mouseButton)
						board.setSpot(r,c,new Piece(5+c*50+60,5+r*50+80,WIDTH/3-10,HEIGHT/3-10,"X",Color.GREEN));
					if (MouseEvent.BUTTON3 == mouseButton)
						board.setSpot(r,c,new Piece(5+c*50+60,5+r*50+80,WIDTH/3-10,HEIGHT/3-10,"O",Color.RED));
				}
	...
	public boolean determineWinner(Graphics window)
	{
		...
		if(winner.equals(""))
		{
			Piece dia0 = (Piece)board.getSpot(0,0);
			Piece dia1 = (Piece)board.getSpot(1,1);
			Piece dia2 = (Piece)board.getSpot(2,2);
			//check for diagonal winner
			if(dia0!=null&&dia1!=null&&dia2!=null)
				if(dia0.getName().equals(dia1.getName())&&dia0.getName().equals(dia2.getName()))
				{
					winner=dia0.getName()+" wins diagonally!";
				}
		}
		
		if(winner.equals(""))
		{
			Piece dia0 = (Piece)board.getSpot(0,2);
			Piece dia1 = (Piece)board.getSpot(1,1);
			Piece dia2 = (Piece)board.getSpot(2,0);
			//check for diagonal winner
			if(dia0!=null&&dia1!=null&&dia2!=null)
				if(dia0.getName().equals(dia1.getName())&&dia0.getName().equals(dia2.getName()))
				{
